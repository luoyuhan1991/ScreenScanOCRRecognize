...
1. **最高优先级**：gui_config.json（用户在GUI中设置的值）
2. **次优先级**：config.yaml（配置文件中的默认值）
3. **最低优先级**：硬编码默认值

### 配置保存时机

- **启动时**：自动加载gui_config.json
- **控件值改变时**：实时更新内存中的配置
- **窗口关闭时**：自动保存当前配置到gui_config.json
- **重置配置按钮**：恢复默认配置并保存
...
├── main.py                    # 原有主程序（保持不变）
├── gui.py                     # 新建：GUI主程序
├── gui_config.json            # 新建：GUI配置文件（自动生成）
├── config.yaml                # 原有配置文件
...
├── src/
│   ├── __init__.py
│   ├── config.py              # 修改：支持GUI配置
│   ├── gui_config_manager.py  # 新建：配置管理器
...
class GUIConfigManager:
    ...
    def apply_to_yaml_config(self, yaml_config):
        """将GUI配置应用到yaml配置对象"""
```
...
1. 创建 `GUIConfigManager` 类
2. 实现配置加载功能
3. 实现配置保存功能
4. 实现配置验证功能
5. 实现配置重置功能
6. 创建 `gui_config.json` 结构
7. 集成配置管理到GUI
...
- `gui_config.json` 保存用户在GUI中设置的值
- `config.yaml` 保存默认配置和隐藏配置项
- 运行时需要将两者合并，GUI配置优先级更高
