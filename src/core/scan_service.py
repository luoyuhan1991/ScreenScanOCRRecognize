...
class ScanService:
    ...
    def _cache_config(self):
        """缓存常用配置（仅在配置变更时刷新）"""
        if hasattr(self, '_config_initialized') and not config.is_dirty():
            return
    ...
    def _is_frame_similar(self, frame_bgr):
        """
        比较当前帧与上一帧的差异，决定是否跳过 OCR。

        Args:
            frame_bgr: numpy BGR 数组

        Returns:
            bool: True 表示帧相似（可跳过 OCR），False 表示帧有变化
        """
        # 缩放为 160x120 灰度图
        small = cv2.resize(frame_bgr, (160, 120), interpolation=cv2.INTER_AREA)
        curr_gray = cv2.cvtColor(small, cv2.COLOR_BGR2GRAY).astype(np.float32)

        if self._prev_frame_gray is None:
            self._prev_frame_gray = curr_gray
            return False

        mse = np.mean((curr_gray - self._prev_frame_gray) ** 2)
        self._prev_frame_gray = curr_gray

        if mse < self.diff_threshold:
            logger.debug(f"帧差 MSE={mse:.2f} < 阈值 {self.diff_threshold}，跳过 OCR")
            return True
        else:
            logger.debug(f"帧差 MSE={mse:.2f} >= 阈值 {self.diff_threshold}，执行 OCR")
            return False
        ...
        try:
            ...
            try:
                ...
                if screenshot is not None:
                    ...
                    # 2.5 帧差检测：跳过无变化帧，复用上次结果
                    if (self.enable_diff_skip
                        ...
                        if self.ocr_engine:
                            ...
                            # 缓存本次结果用于帧差跳过时复用
                            self._prev_ocr_results = result['ocr_results']
                            self._prev_matches = result['matches']
                ...
                # 显式释放截图对象，减少内存占用
                if screenshot is not None and self.explicit_image_cleanup:
                    try:
